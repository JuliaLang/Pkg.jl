...
**Keyword arguments:**
  ...
  - `compat_modifier::Union{Function, Nothing}=nothing`: [EXPERIMENTAL] an optional function for modifying `[compat]` entries inside the test sandbox project

!!! compat "Julia 1.3"
    `julia_args` and `test_args` requires at least Julia 1.3.

!!! compat "Julia 1.7"
    `compat_modifier` requires at least Julia 1.7.

!!! note
    The `compat_modifier`  keyword argument is an experimental feature. The
    behavior is subject to change or removal in minor or patch releases of
    Julia.
