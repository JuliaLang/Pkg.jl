...
Note that the option `--inline=no` is passed  to the new process if that
option was also given to the currently running Julia, i.e., if
`Base.JLOptions().can_inline` is false.
"""
const test = API.test
