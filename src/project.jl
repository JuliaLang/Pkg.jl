...
function Project(raw::Dict; file=nothing)
    ...
    project.entrypath     = get(raw, "path", nothing)::Union{String, Nothing}
    if project.entrypath === nothing
        project.entrypath = get(raw, "entrypath", nothing)::Union{String, Nothing}
    end
...
function destructure(project::Project)::Dict
    ...
    entry!("entrypath",     project.entrypath)
