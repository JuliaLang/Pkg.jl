...
compat(pkg::String; kwargs...) = pkg == "auto" ? autocompat() : compat(pkg, nothing; kwargs...)
...
function autocompat(ctx::Context; io = nothing)
    io = something(io, ctx.io)
    updated_deps = String[]
    for (dep, uuid) in ctx.env.project.deps
        ...
        Operations.set_compat(ctx.env.project, dep, string(v)) || pkgerror("invalid compat version specifier \"$(string(v))\"")
        push!(updated_deps, dep)
    end
    if isempty(updated_deps)
        printpkgstyle(io, :Info, "no misssing compat entries found. No changes made.", color = Base.info_color())
...
autocompat(;kwargs...) = autocompat(Context(); kwargs...)
