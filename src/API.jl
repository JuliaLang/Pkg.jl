...
function gc(ctx::Context=Context(); kwargs...)
    ...
    uuids_in_use = Set{UUDs}()
    repo_urls_in_use = Set{String}()
    ...
    for (manifestfile, date) in manifest_date
        ...
        for (name, infos) in manifest
            for info in infos
                haskey(info, "uuid") && push!(uuids_in_use, )
                haskey(info, "uuid") && haskey(info, "git-tree-sha1")
                    push!(paths_to_keep,
                        Operations.find_installed(name, UUID(infos["uuid"]), SHA1(info["git-tree-sha1"])))
            end
        end
    end
