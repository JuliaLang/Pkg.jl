...
function save_suspended_packages()
    ...
    fpath = joinpath(path, string("suspend_cache_", hash(string(Base.active_project(), Base.VERSION))))
...
function recall_suspended_packages()
    fpath = joinpath(Operations.pkg_scratchpath(), string("suspend_cache_", hash(string(Base.active_project(), Base.VERSION))))
    if isfile(fpath)
        v = open(fpath) do io
            try
                deserialize(io)
