...
function _is_dev_version(v::VersionNumber)
    return length(v.prerelease) >= 1 && v.prerelease[1] == "DEV"
end

function _prune_dev_version(v::VersionNumber)
    if _is_dev_version(v)
        return VersionNumber(v.major, v.minor, v.patch, ("DEV",), ())
    end
    return v
end

function _generated_by_julia(v::VersionNumber)
    return "# Generated by Julia version $(_prune_dev_version(v))\n\n"
end
...
function write_manifest(manifest::Dict, manifest_file::AbstractString)
    ...
    print(io, _generated_by_julia(Base.VERSION))
