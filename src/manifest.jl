...
function validate_manifest(julia_version::Union{Nothing, VersionNumber}, project_hash::Union{Nothing, SHA1}, manifest_format::VersionNumber, stage1::Dict{String, Vector{Stage1}}, other::Dict{String, Any}, f_or_io::Union{String, IO})
    # Create file info string for error messages
    file_info = f_or_io isa String ? " in file `$f_or_io`" : ""
            ...
            for (name, uuid) in deptype
                ...
                if !isext
                    if dep_entry === nothing
                        pkgerror(
                            "Invalid manifest$(file_info): `$(entry.name)=$(entry_uuid)` depends on `$name=$uuid`, ",
                            "but no such entry exists in the manifest."
                        )
                    end
    ...
    # Validate manifest entries for consistency
    for (entry_uuid, entry) in deps
        # Packages with path should not have tree_hash (issue #4367)
        if entry.path !== nothing && entry.tree_hash !== nothing
            pkgerror(
                "Invalid Manifest.toml$(file_info): Package `$(entry.name)=$(entry_uuid)` has both `path` and `git-tree-sha1` fields. ",
                "Path-based dependencies should not have tree hashes. ",
                "Note that Manifest.toml files should only be generated by Pkg and not manually edited."
            )
        end

        # Packages tracking repositories should have valid tree_hash (issue #4367)
        if entry.repo.source !== nothing && entry.tree_hash === nothing
            pkgerror(
                "Invalid Manifest.toml$(file_info): Package `$(entry.name)=$(entry_uuid)` tracks a repository but has no `git-tree-sha1` field. ",
                "Repository-based dependencies must have valid tree hashes. ",
                "Note that Manifest.toml files should only be generated by Pkg and not manually edited."
            )
        end
    end
...
function Manifest(raw::Dict{String, Any}, f_or_io::Union{String, IO})::Manifest
    ...
    return validate_manifest(julia_version, project_hash, manifest_format, stage1, other, f_or_io)
end
...
function read_manifest(f_or_io::Union{String, IO})
    raw = try
        ...
        if e isa TOML.ParserError
            # Add file path info when not already included in TOML error
            file_info = f_or_io isa String && getfield(e, :filepath) === nothing ? " `$f_or_io`" : ""
            pkgerror("Could not parse manifest$(file_info): ", sprint(showerror, e))
        end
        rethrow()
    end
