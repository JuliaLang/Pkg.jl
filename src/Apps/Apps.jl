...
function _resolve(manifest::Manifest, pkgname=nothing)
    for (uuid, pkg) in manifest.deps
        ...
        base_sourcepath = source_path(app_manifest_file(), pkg)

        # Collect unique environments to resolve
        resolved_environments = Set{String}()
        for (appname, app) in pkg.apps
            # TODO: Possible collision if multiple apps with the same basename for the app path
            app_env = app.path === nothing ?
                joinpath(app_env_folder(), pkg.name) :
                joinpath(app_env_folder(), pkg.name, basename(app.path))

            if app_env in resolved_environments
                continue
            end

            projectfile = joinpath(app_env, "Project.toml")
            push!(resolved_environments, app_env)

            sourcepath = joinpath(base_sourcepath, something(pkg.path, "."))
            ...
            if isfile(original_project_file)
                ...
                name = project_data["name"]::String
                project_data["entryfile"] = joinpath(sourcepath, "src", "$(name).jl")
            ...
            ctx = Context(env=EnvCache(joinpath(app_env, "Project.toml")))
            Pkg.resolve(ctx)
        end

        # TODO: Julia path
        generate_shims_for_apps(pkg.name, pkg.apps, base_sourcepath, joinpath(Sys.BINDIR, "julia"))
    end
...
function generate_shims_for_apps(pkgname, apps, sourcepath, julia)
    for (_, app) in apps
        generate_shim(pkgname, app, sourcepath, julia)
    end
end

function generate_shim(pkgname, app::AppInfo, sourcepath, julia)
    validate_package_name(pkgname)
    validate_app_name(app.name)
    validate_submodule_name(app.submodule)

    # Determine environment path and module name
    if app.path !== nothing
        # Pathed app: use PackageName/PathDirName environment
        absolute_app_path = joinpath(sourcepath, app.path)
        path_dirname = basename(absolute_app_path)
        app_env = joinpath(app_env_folder(), pkgname, path_dirname)
        # Read module name from the pathed Project.toml
        app_project_path = joinpath(absolute_app_path, "Project.toml")
        if isfile(app_project_path)
            app_project = Pkg.Types.read_project(app_project_path)
            base_module_name = app_project.name
        else
            error("Project.toml not found in path: $(absolute_app_path)")
        end
    else
        # Regular app: use PackageName environment
        app_env = joinpath(app_env_folder(), pkgname)
        base_module_name = pkgname
    end

    module_spec = app.submodule === nothing ? base_module_name : "$(base_module_name).$(app.submodule)"
    ...
    content = if Sys.iswindows()
        windows_shim(julia_escaped, module_spec_escaped, app_env)
    else
        bash_shim(julia_escaped, module_spec_escaped, app_env)
    end
