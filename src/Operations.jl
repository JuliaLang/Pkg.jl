...
function source_path(manifest_file::String, pkg::Union{PackageSpec, PackageEntry}, julia_version = VERSION)
    ...
    pkg.entryfile   !== nothing ? pkg.entryfile :
...
function is_instantiated(env::EnvCache, workspace::Bool=false; platform = HostPlatform())::Bool
    ...
    if env.pkg !== nothing
        ...
        if idx === nothing
            push!(pkgs, Types.PackageSpec(name=env.pkg.name, uuid=env.pkg.uuid, version=env.pkg.version, path=dirname(env.project_file), entryfile=env.project.entryfile))
        end
    else
