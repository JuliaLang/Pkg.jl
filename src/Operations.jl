...
function source_path(ctx, pkg::PackageSpec)
    return is_stdlib(pkg.uuid, ctx.julia_version) ? Types.stdlib_path(pkg.name) :
...
function deps_graph(ctx::Context, uuid_to_name::Dict{UUID,String}, reqs::Resolve.Requires, fixed::Dict{UUID,Resolve.Fixed})
    ...
    while true
        ...
        for uuid in unseen
            ...
            if is_unregistered_stdlib(uuid) || is_stdlib(uuid, ctx.julia_version)
...
function load_urls(ctx::Context, pkgs::Vector{PackageSpec})
    ...
    for pkg in pkgs
        uuid = pkg.uuid
        pkg.version isa VersionNumber || throw(ErrorException("The manifest file you are using was most likely generated by a different version of Julia and is not compatible with this Julia version"))
...
function is_package_downloaded(ctx, pkg::PackageSpec)
    ...
    if sourcepath === nothing
        msg = string(
            "This manifest file was generated by a different version of Julia and thus is not ",
            "compatible with this version of Julia. To generate a manifest file that is ",
            "compatible with this version of Julia, run `import Pkg; Pkg.resolve()`. Please ",
            "note, however, that this will re-resolve the package versions in the environment.",
        )
        pkgerror(msg)
    end
