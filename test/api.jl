...
@testset "Pkg.precompile" begin
    ...
    isolate(loaded_depot = true) do;
        cd_tempdir() do tmp
            ...
            @testset "pidlocked precompile" begin
                ...
                # 10 s precompilation of `SlowPrecompile` is running, so they
                # must load Pkg from the cache (as the inherited options allow).
                cmd = addenv(
    ...
    isolate() do;
        cd_tempdir() do tmp
            ...
            @test_throws PkgError Pkg.precompile("DoesNotExist") # fail to find a nonexistant dep in an empty env
