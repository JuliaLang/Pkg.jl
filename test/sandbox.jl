...
@testset "Basic `test` sandboxing" begin
    ...
    temp_pkg_dir() do project_path; mktempdir() do tmp
        ...
        test_test("Unregistered") do
            json = get(Pkg.Contexts.Context().env.manifest, UUID("682c06a0-de6a-54ab-a142-c8b1cf79cde6"), nothing)
            @test json !== nothing
    ...
    temp_pkg_dir() do project_path; mktempdir() do tmp
        ...
        test_test("Foo") do
            x = get(Pkg.Contexts.Context().env.manifest, UUID("7876af07-990d-54b4-ab0e-23690620f79a"), nothing)
            @test x !== nothing
